...
public class GroupPrivilegeUpdatePropagationTests
{
    [Fact]
    public void PrivilegeUpdates_AreQueuedOnParticipants()
    {
        ...
        server.GroupManager.SetPrivileges(inviter.Uid, invitee.Uid, GroupPrivileges.Invite);
        updateWorld.Invoke(server, null);
        ...
        Assert.NotNull(ev1);
        Assert.NotNull(ev2);
        Assert.IsType<ParticipantEvent.GroupPrivilegeUpdate>(ev1!);
        Assert.IsType<ParticipantEvent.GroupPrivilegeUpdate>(ev2!);
    }
}
